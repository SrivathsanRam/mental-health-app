...
export default function Onboarding() {
  ...
  const [selectedGoals, setSelectedGoals] = useState<boolean[]>(Array(6).fill(false)); // Track selected goals
  const [showDatePicker, setShowDatePicker] = useState<boolean>(false);

  const handleSave = async () => {
    const selectedGoalTitles = goalTitles.filter((_, index) => selectedGoals[index]);
    const selectedGoalImages = goalImages.filter((_, index) => selectedGoals[index]);

    if (!alias || !dob || selectedGoalTitles.length === 0) {
      Alert.alert('Error', 'Please fill all fields and select at least one goal');
      return;
    }
    ...
    try {
      ...
      await AsyncStorage.setItem('goals', JSON.stringify({ titles: selectedGoalTitles, images: selectedGoalImages }));
